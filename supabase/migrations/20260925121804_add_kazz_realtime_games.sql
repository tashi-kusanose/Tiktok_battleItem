-- Independent live-game data. Guest capabilities are verified by the Edge API.
create table public.kg_rooms (
 id uuid primary key default gen_random_uuid(), code text not null unique,
 host_hash text not null unique, name text not null check(char_length(name) between 1 and 40),
 channel uuid not null default gen_random_uuid(), revision bigint not null default 0,
 locked boolean not null default false, closed boolean not null default false,
 state jsonb not null default '{}', created_at timestamptz not null default now(),
 expires_at timestamptz not null default now()+interval '48 hours'
);
create index kg_rooms_expiry on public.kg_rooms(expires_at);
create table public.kg_players (
 id uuid primary key default gen_random_uuid(), room_id uuid not null references public.kg_rooms(id) on delete cascade,
 token_hash text not null, name text not null check(char_length(name) between 1 and 20),
 banned boolean not null default false, round_id text, answer jsonb, card jsonb,
 marks jsonb not null default '[]', bingo boolean not null default false,
 joined_at timestamptz not null default now(), answered_at timestamptz,
 unique(room_id, token_hash), unique(room_id,name)
);
create index kg_players_room on public.kg_players(room_id,joined_at);
create table public.kg_limits (key text primary key, hits int not null, expires_at timestamptz not null);
create index kg_limits_expiry on public.kg_limits(expires_at);
alter table public.kg_rooms enable row level security;
alter table public.kg_players enable row level security;
alter table public.kg_limits enable row level security;
revoke all on public.kg_rooms,public.kg_players,public.kg_limits from anon,authenticated;
grant all on public.kg_rooms,public.kg_players,public.kg_limits to service_role;

create function public.kg_rand(n int) returns int language plpgsql volatile set search_path='' as $$
declare b bytea; v bigint; ceiling bigint;
begin
 if n<1 then raise exception '抽選候補がありません'; end if;
 ceiling:=4294967296-(4294967296%n);
 loop
...
revoke all on function public.kg_rand(int) from public,anon,authenticated;
grant execute on function public.kg_rand(int) to service_role;

create function public.kg_card() returns jsonb language plpgsql volatile set search_path='' as $$
declare a int[]:=array_fill(0,array[25]); pool int[]; c int; r int; j int; v int;
begin
 for c in 0..4 loop
 select array_agg(g) into pool from generate_series(c*15+1,c*15+15) g;
 for r in 0..4 loop
 j:=public.kg_rand(array_length(pool,1))+1; v:=pool[j]; pool:=array_remove(pool,v); a[r*5+c+1]:=v;
 end loop;
 end loop;
 a[13]:=0; return to_jsonb(a);
end $$;
revoke all on function public.kg_card() from public,anon,authenticated;
grant execute on function public.kg_card() to service_role;

create function public.kg_api(p_action text,p_code text,p_hash text,p_data jsonb,p_rate text)
...
begin
 ...
 if p_action not in ('create','snapshot','join') then
   ...
   if p_action not in ('answer','mark','claim','pull') and not host then raise exception '配信者だけが操作できます'; end if;
   if p_action in ('answer','mark','claim','pull') and host then raise exception '参加用画面から操作してください'; end if;
   if p_action in ('answer','mark','claim','pull','draw','close') and (p_data->>'round') is distinct from rid then raise exception 'ゲームが切り替わりました。画面の更新をお待ちください'; end if;
   ...
   if p_action='start' then
     ...
     if kind is null or kind not in ('question','either','ranking','comment','mission','bingo','roulette','gacha','lottery','quiz','rps','dice','number') then raise exception 'ゲームが無効です'; end if;
     ...
     if kind in ('either','ranking','roulette','gacha','quiz') then
       opts:=cfg->'options';
       if jsonb_typeof(opts) is distinct from 'array' then raise exception '選択肢が必要です'; end if;
       n:=jsonb_array_length(opts);
       if n<2 or n>20 or (kind='either' and n<>2) or (kind='quiz' and n<>4) then raise exception '選択肢の数を確認してください'; end if;
     ...
     if not me.marks @> jsonb_build_array(n) then
       update public.kg_players set marks=marks||jsonb_build_array(n) where id=me.id; mutated:=true;
     end if;
 ...
 'answer',case when s->>'status'='closed' or s->>'kind' in ('question','comment','either','ranking','mission','gacha','dice') then p.answer else null end,
...
revoke all on function public.kg_api(text,text,text,jsonb,text) from public,anon,authenticated;
grant execute on function public.kg_api(text,text,text,jsonb,text) to service_role;
