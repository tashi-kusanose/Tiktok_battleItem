...
begin
 ...
 if not host and me.id is not null and kind='bingo' and (me.card is null or me.round_id is distinct from rid) then
   update public.kg_players set round_id=rid,answer=null,marks='[]',bingo=false,answered_at=null,card=public.kg_card() where id=me.id;
   select * into me from public.kg_players where id=me.id;
 end if;
